export const EMAIL = 'lindsay.ferguson@reqres.in';
export const SENHA = 'petscare';
...
export default async (parametros) => {
  ...
  await fetch('https://reqres.in/api/login', {
    ...
    .then((response) => {
      if (response.status === 200) {
        response.text().then(function () {
          parametros.navigation.navigate('Produtos');
          alert(`Login efetuado com sucesso`);
        });
      } else {
        ShowModal(
          `Usuário ou senha inválidos => código: ${response.status}`,
          parametros.modalVisible,
    ...
    .catch((e) => {
      ShowModal(
        'Não foi possivel executar o login =>' + e,
        parametros.modalVisible,
        parametros.description
      );
      parametros.activity(false);
